"""Real licensed CoreHost integration checks; Python standard library only.

python integration.py --writer path/to/GeoModelBridge.ProWriter.exe --work path/to/work
...
root = Path(tempfile.mkdtemp(prefix='arcgis-pro-integration-', dir=args.work.resolve()))
...
assert report['texture_coordinate_policy']['mapping'] == 'S=U; T=1-V'
assert report['texture_coordinate_policy']['image_rows_reordered'] is False
...
# Finite but unrepresentable coordinates fail in the geodatabase write, after staging exists.
...
assert staging_observed, 'Expected to observe an actual temporary geodatabase before the write failure'
assert not (root / 'overflow.gdb').exists()
assert not list(root.glob('*.gmb-*.gdb'))
results.append(dict(case='geodatabase-write-failure-cleans-only-staging', passed=True, staging_observed=True, error=stderr.strip()))

# Expected hashes are actually checked, rather than merely counting rows.
corrupt_expected = copy.deepcopy(report)
corrupt_expected['verification']['checks'][0]['signature']['patches'][0]['texture_sha256'] = '0' * 64
