...
Python 语言兼容目标为 3.9 及以上；实际验证解释器见[本版验证记录](validation-v0.1.12.md)。不需要 `pip install`，将完整发布目录的 `python` 加入调用脚本的 `sys.path` 即可。无需修改 ArcGIS 的 Python 环境或安装第三方包。Ubuntu 使用同一套库，程序名不带 `.exe`，运行库布局见[部署指南](build-and-release.md)。
...
V0.1.12 对选中的目录型贴图路径、错误父目录和访问失败返回 `ConversionError(code="PROCESS_FAILED", exit_code=3)`，通过 `error.diagnostics` 中的 `TEXTURE_READ_ERROR` 获取路径与原因。`missing_textures="material-color"` 仍支持真正缺图的情况；宿主应提示修正读错误，不能把所有贴图诊断都当成可忽略的警告。Python 函数签名保持不变。
...
python tests/python_client_integration_test.py --install-dir releases/V0.1.12 --work artifacts/new-python-client-check
```
