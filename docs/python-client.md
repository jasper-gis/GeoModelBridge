...
Python 语言兼容目标为 3.9 及以上；实际验证解释器见[本版验证记录](validation-v0.1.13.md)。不需要 `pip install`，将完整发布目录的 `python` 加入调用脚本的 `sys.path` 即可。无需修改 ArcGIS 的 Python 环境或安装第三方包。Ubuntu 使用同一套库，程序名不带 `.exe`，运行库布局见[部署指南](build-and-release.md)。
...
python tests/python_client_integration_test.py --install-dir releases/V0.1.13 --work artifacts/new-python-client-check
```
