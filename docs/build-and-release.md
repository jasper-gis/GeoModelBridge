...
[← 返回项目首页](../README.md) · [原生后端](../backends/native-filegdb/README.md) · [验证记录](validation-v0.1.6.md)
...
| 缺贴图或材质语义不支持 | 查看报告诊断，可用重复的 `--texture-dir DIR` 补充贴图搜索目录；不跳过未知材质或无效数据 |
...
验证记录明确区分核心测试、原生数据库回读、可搬迁部署与目标软件显示验收。自动回读通过不等于完成三维外观验收。[V0.1.6 记录](validation-v0.1.6.md) · [验收边界](acceptance.md)
