...
public sealed class EngineService
{
    ...
    public async Task<ConversionResult> ConvertAsync(ConversionSettings settings, IProgress<EngineEvent>? progress = null)
    {
        ...
        try
        {
            ...
            var report = ReportVerifier.Verify(await File.ReadAllTextAsync(reportPath).ConfigureAwait(false), settings);
    ...
    private async Task<ProbeResult> CheckEngineAsync(IProgress<EngineEvent>? progress)
    {
        ...
        if (result.ExitCode != 0 || (version != "GeoModelBridge V" + ProductInfo.Version && version != "GeoModelBridge " + ProductInfo.Version))
    ...
    private static async Task ReadAsync(StreamReader reader, StringBuilder captured, EngineEventKind kind, IProgress<EngineEvent>? progress)
    {
        const int captureLimit = 262144, previewLimit = 2048, visibleLines = 12;
        var buffer = new char[4096];
        var preview = new StringBuilder(previewLimit);
        long lineCount = 0;
        bool lineHasContent = false, lineTruncated = false, previousWasCr = false;
        void PublishLine()
        {
        ...
        while ((count = await reader.ReadAsync(buffer.AsMemory()).ConfigureAwait(false)) > 0)
        {
            ...
            if (captured.Length > captureLimit) captured.Remove(0, captured.Length - captureLimit);
    ...
    private static async Task<string?> ReadFailureSummaryAsync(string reportPath)
    {
        if (File.Exists(reportPath))
        {
            try
            {
                ...
                {
                    var summary = ReportSummaryFormatter.Parse(await File.ReadAllTextAsync(reportPath).ConfigureAwait(false));
    ...
    private static string ProcessDetails(ProcessResult result) => string.Join(Environment.NewLine,
        new[] { string.IsNullOrWhiteSpace(result.StandardError) ? "" : "错误输出：" + Tail(result.StandardError, 2000),
            string.IsNullOrWhiteSpace(result.StandardOutput) ? "" : "程序输出：" + Tail(result.StandardOutput, 2000) }.Where(value => value.Length > 0));
